...
namespace ItemRecovery
{
    public class ModEntry : Mod
    {
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            // helper.Events.Input.ButtonPressed += this.OnButtonPressed2;
            // helper.Events.Display.MenuChanged += this.OnMenuChanged;
            helper.Events.Display.MenuChanged += this.OnMenuChanged2;
        }
        ...
        private void OnMenuChanged2(object sender, MenuChangedEventArgs e)
        {
            if (e.NewMenu is ShopMenu newMenu)
            {
                ...
                if (!same)
                {
                    this.Helper.Events.Input.ButtonPressed += this.OnButtonPressed2;
                }
            }
        ...
        private void OnButtonPressed2(object sender, ButtonPressedEventArgs e)
        {
            ...
            // this.Monitor.Log($"{Game1.player.Name} pressed {e.Button}. debug", LogLevel.Debug);
            receiveLeftClick(menu, Game1.getMouseX(), Game1.getMouseY(), true);
        }
        
        public void receiveLeftClick(ShopMenu menu, int x, int y, bool playSound = true)
        {
            for (int index = 0; index < menu.forSaleButtons.Count; ++index)
            {
                    ...
                    menu.forSaleButtons[index].containsPoint(x, y))
                {
                    ...
                    if (menu.forSale[num] != null)
                    {
                        ...
                        // IReflectedMethod method = reflection.GetMethod(menu, "tryToPurchaseItem", true);
                        // if (numberToBuy > 0 && method.Invoke<bool>(menu.forSale[num], menu.heldItem, numberToBuy, x, y, num)) 
        ...
        //         return;
        //
        //     foreach (ISalable item in menu.itemPriceAndStock.Keys.ToList())
        //     {
        //         if (((Item)item).isLostItem)
        //             menu.itemPriceAndStock[item][0] = item.salePrice();
        //     }
        // }
        ...
        //         return;
        //
        //     this.Monitor.Log($"{Game1.player.Name} pressed {e.Button}.sss", LogLevel.Debug);
        //
        //     foreach (ISalable i in menu.itemPriceAndStock.Keys.ToList())
        //     {
        //         this.Monitor.Log($"Item: {i.Name} Price: {i.salePrice()}", LogLevel.Debug);
        //         foreach (int v in menu.itemPriceAndStock[i])
        //         {
        //             this.Monitor.Log($"Ints: {v}", LogLevel.Debug);
        //         }
        //     }
        //
        //     Item item = (Item)menu.hoveredItem;
        //     if (item != null && item.isLostItem)
        //     {
        //         item.isLostItem = false;
        //         //this.Monitor.Log($"Item: {item.Name} Price: {item.salePrice()}", LogLevel.Debug);
        //     }
        //
